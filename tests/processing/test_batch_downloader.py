...
def test_get_toy_batch_generator():
    ...
    for da_batch in batch_generator:
        ...
        print(
            f"depth-start: {da_batch.depth.values[0]}, depth-end: {da_batch.depth.values[-1]}"
        )
        assert np.isclose(da_batch.depth.values[0], 1)
        assert np.isclose(da_batch.depth.values[-1], 10)

        print(
            f"time-start: {da_batch.time.values[0]}, time-end: {da_batch.time.values[-1]}"
        )
        assert da_batch.time.values[0] == np.datetime64("2025-01-01T00:00:00")

        print(
            f"frequency-start: {da_batch.frequency.values[0]}, frequency-end: {da_batch.frequency.values[-1]}"
        )
...
def test_get_s3_batch_generator():
    """
    This is a functional test getting real dataset from s3 and checking batch values.
    """
    batch_downloader = BatchDownloader(
    ...
    print(
        f"number_of_batches_in_cruise: {list(batch_generator._batch_selectors.selectors)}"
    )
    for da_batch in batch_generator:
        ...
        assert da_batch.time.values[0].astype("datetime64[s]") == np.datetime64(
            "2007-07-11T18:20:33.657573888"
        ).astype("datetime64[s]")

        print(
            f"frequency-start: {da_batch.frequency.values[0]}, frequency-end: {da_batch.frequency.values[-1]}"
        )
        assert da_batch.frequency.values[0] == 18_000.0
        assert da_batch.frequency.values[-1] == 200_000.0
